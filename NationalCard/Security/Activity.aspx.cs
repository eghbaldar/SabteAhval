...
public partial class Activity : System.Web.UI.Page
{
    ...
    private void SetNew()
    {
        if (this.pageStatus == HNP.Base.Enum.PageStatus.Edit || this.pageStatus == HNP.Base.Enum.PageStatus.New)
            return;
        this.lblActivityID.Enabled = true;
        this.lblActivityID.Text = "";
        this.drpActivityTypeID.Enabled = true;
        this.txtActivityName.Enabled = true;
        this.txtActivityName.Text = "";
        this.txtActivityDesc.Enabled = true;
        this.txtActivityDesc.Text = "";
    ...
    {
        this.lblActivityID.Enabled = false;
        this.drpActivityTypeID.Enabled = false;
        this.txtActivityName.Enabled = false;
        this.txtActivityDesc.Enabled = false;
        this.pageStatus = HNP.Base.Enum.PageStatus.None;
        this.RefreshPageStatus(this.pageStatus);
        this.loadControls();
    }
    ...
    private void SetDelete(int ItemIndex)
    {
        ...
        else
        {
            ...
            HNP.Security.Difinition.Activity obj;
            obj = HNP.Security.Difinition.SecurityCollections.Activity[key];
            if (obj != null)
                if (obj.Delete())
                {
                    LoadData();
    ...
    private void SetSave()
    {
        if (this.pageStatus == HNP.Base.Enum.PageStatus.Edit || this.pageStatus == HNP.Base.Enum.PageStatus.New)
        {
            ...
            {
              HNP.Security.Difinition.Activity obj = new HNP.Security.Difinition.Activity();
            ...
            else
            {
                ...
                HNP.Security.Difinition.Activity obj = HNP.Security.Difinition.SecurityCollections.Activity[key];
                this.ReadControlsValue(obj);
                if (obj.Update())
                {
    ...
    private void ReadControlsValue(HNP.Security.Difinition.Activity obj)
    {
        if (pageStatus != HNP.Base.Enum.PageStatus.New)
            obj.ActivityID = Int16.Parse(this.lblActivityID.Text);
        obj.ActivityType = SecurityCollections.ActivityType[short.Parse(this.drpActivityTypeID.SelectedValue)];
        obj.ActivityName = this.txtActivityName.Text;
        obj.ActivityDesc = this.txtActivityDesc.Text;
    }

    private void LoadData()
    {
        this.gvMaster.DataSource = HNP.Security.Difinition.SecurityCollections.Activity.GetAll;
    ...
    private void loadControls(Int16 key)
    {
        ...
        if (obj != null)
        {
        this.lblActivityID.Text = obj.ActivityID.ToString();
        this.drpActivityTypeID.SelectedValue = obj.ActivityType.ActivityTypeID.ToString();
        this.txtActivityName.Text = obj.ActivityName.ToString();
        this.txtActivityDesc.Text = obj.ActivityDesc.ToString();
        }
    }
